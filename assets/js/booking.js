...
$(document).ready(function () {
    ...
    $(document).on("submit", ".hg-booking-form", function (e) {
        ...
        let roomCategory = form.find('select[name="room_category"]').val() || form.find('input[name="room_category"]').val() || "";
        let waMessage = "";
...
I would like to book a stay in *Jim Corbett*.
...
🏨 *Selected Resort*: ${service}` +
(roomCategory ? `\n🛏️ *RM Category*: ${roomCategory}` : '') +
`\n💬 *Message/Notes*: ${message}

Please confirm availability and share the best package rates. Thank you!`;
        }
            ...
            ">
                ...
                ">
                    ...
                    ">
                        ...
                        <strong>Resort/Stay:</strong> ${booking.service}<br>
                        ${booking.roomCategory ? `<strong>RM Category:</strong> ${booking.roomCategory}<br>` : ''}
    ...
    function updateStayRoomDropdown(hotelName, selectedRoom) {
        ...
        rmSelect.empty();
        const rooms = window.CORBETT_HOTEL_ROOMS[hotelName] || ["Standard Room"];
        rooms.forEach(function (r) {
            const isSel = (selectedRoom && selectedRoom.trim().toLowerCase() === r.trim().toLowerCase()) ? 'selected' : '';
            rmSelect.append(`<option value="${r}" ${isSel}>${r}</option>`);
        });
        if ($.fn.niceSelect) {
            try {
                rmSelect.niceSelect('update');
            } catch(e) {}
        }
    }

    // When Stay changes in sidebar form
    $(document).on('change', '#service', function () {
        const selectedHotel = $(this).val();
        updateStayRoomDropdown(selectedHotel);
    });

    // Initialize room category for initially selected hotel on page load
    if ($('#service').length && $('#room_category').length) {
        updateStayRoomDropdown($('#service').val());
    }

    // Quick Book button or Room Chip clicked on any Hotel Card
    $(document).on('click', '.book-stay-btn, .hotel-room-chip', function (e) {
        e.preventDefault();
        ...
        if (hotel) {
            if ($('#service').length) {
                ...
                updateStayRoomDropdown(hotel, room);

                // Smooth scroll to stay booking form
                const target = $('#stay-booking');
                if (target.length) {
                    $('html, body').animate({
                    ...
                    target.css({
                        'box-shadow': '0px 0px 30px rgba(253, 126, 33, 0.6)',
                        'border-color': '#FD7E21',
